...
describe('Double Down Logic', () => {
  describe('canDouble', () => {
    ...
    it('should work with default allowDoubleAfterSplit parameter (false)', () => {
      const hand: Hand = {
      ...
      // Default should NOT allow double after split (standard casino rule)
      expect(canDouble(hand)).toBe(false);
    });
  });
  ...
  describe('doubleDownHand', () => {
    ...
    it('should not allow double after split by default', () => {
      ...
      // Default does NOT allow double after split
      expect(() => doubleDownHand(hand)).toThrow('Cannot double down this hand');
    });
  });
